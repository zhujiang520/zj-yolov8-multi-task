...
from ultralytics.yolo.utils.multitask import multitask_loss_bucket
...
class DetectionSegmentationTrainer(BaseTrainer):
    ...
    def _head_layer_index(self, task_idx):
        """Sequential index in ``model.model`` for ``preds[task_idx]`` (heads in forward order).

        ``count - len(labels_list)`` only works when the YAML has exactly as many heads as ``labels_list``
        entries. If the graph still contains an extra Segment head (e.g. BDD 3-head YAML with 2 tasks),
        negative indexing points at the wrong module and detection loss sees 8000 vs 8400 anchors.
        """
        m = de_parallel(self.model)
        head_layers = [i for i, ly in enumerate(m.model) if isinstance(ly, (Detect, Segment))]
        if task_idx >= len(head_layers):
            raise IndexError(
                f'multi-task task index {task_idx} out of range: model has {len(head_layers)} Detect/Segment '
                f'heads but needs an index for labels_list[{task_idx}]')
        return head_layers[task_idx]

    def criterion(self, preds, batch, name=None, count=None):
        """Compute loss for YOLO prediction and ground-truth."""
        head_idx = self._head_layer_index(count)
        if 'det' in name:
            self.compute_loss = Loss(de_parallel(self.model), head_idx)
        elif 'seg' in name:
    ...
    def label_loss_items(self, loss_items=None, prefix='train',task=None):
        ...
        if loss_items is not None:
            ...
            for name in self.data['labels_list']:
                loss_names.extend(self.loss_names[multitask_loss_bucket(name)])
    ...
    def label_loss_items_val(self, loss_items=None, prefix='val', task=None):
        ...
        # Not needed for classification but necessary for segmentation & detection
        if loss_items is not None:
            keys = [f'{prefix}/{x}' for x in self.loss_names[multitask_loss_bucket(task)]]
