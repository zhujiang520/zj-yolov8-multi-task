...
from ultralytics.yolo.utils.multitask import assert_model_outputs_cover_tasks, n_multi_tasks
        ...
        for batch_i, batch in enumerate(bar):
            ...
            with dt[1]:
                if self.args.task == 'multi':
                    ...
                    assert_model_outputs_cover_tasks(preds_list, self.data['labels_list'])
                else:
            ...
            with dt[2]:
                if self.training:
                    if self.args.task == 'multi':
                        # Extra heads are ignored; loss buffers follow ``len(labels_list)`` supervised tasks.
                        n_tasks = n_multi_tasks(self.data)
                        for i in range(n_tasks):
                            self.loss[i] += trainer.criterion(
                                preds_list[i], batch[i], self.data['labels_list'][i], i)[1]
                    else:
            ...
            with dt[3]:
                if self.args.task == 'multi':
                    preds_list_post = []
                    for i in range(n_multi_tasks(self.data)):
                        preds = preds_list[i]
